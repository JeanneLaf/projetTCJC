...
# ╔═╡ 37352fe0-bad7-4c3d-9266-6bdd550dd717
...
!!! info "Proposition 1"
	##### Ajouts:
	---
	- **Un** départ du parcours 14 *(ULaval via Shannon)* le matin à **6h20**
...
# ╔═╡ c43eb20b-c804-4c5b-af97-cd358b650a06
begin
...
function parse_itinerary(itinerary::JSON3.Object; MAP_BOUNDS=[(46.75,-71.5),(46.96,-71.4)])
	...
	for leg in legs
		...
		if leg[:mode]=="WALK"
			...
			text = "- Prendre le bus "*leg[:route][:shortName]*" à "*display_time
			s*=text*"\n"
			flm.PolyLine(        
			    locations=path_coordinates,
			    popup=text,
	...
	display_time = Dates.format(dt, "HH:MM")
	s*="- heure d'arrivée: "*display_time
	
	return Dict(:map => m, :text => s, :duree => duration)
end
end
...
WideCell(
	...
	<div class="pourquoi">
		<h5>Pourquoi ces parcours</h5>
		 <hr>
		<ul>
		<li> <b>Évite les artères problématiques du aux travaux du tramway</b><br>
...
WideCell(@htl(
	...
	<div style="display: flex; gap: 20px; justify-content: space-between;">
		<div style="flex: 1; min-width: 200px;">
			...
			$(@bind depart Select([p for p in keys(Destinations)]))
		</div>
		<div style="flex: 1; min-width: 200px;">
			<h6>Destination</h6>
...
begin
	response = callAPI(Destinations[depart],Destinations[arrivee],heure)
	if response==0
		text ="Aucun itinéraire trouvée\n Essayez de modifier l'heure de départ"
		map = nothing
		duree = nothing
	else
		itinerary = parse_itinerary(response)
		text = itinerary[:text]
		map = itinerary[:map]
	...
	<div style="display: flex; justify-content: space-between;">
		...
		<div style="flex: 1; min-width: 500px;white-space:pre-line;background-color:#dce8fa;padding:20px">
			<h5>Itinéraire</h5><hr width="230px">
			$(text)<br>
			Durée du trajet: <b>$(duree)</b>
		</div>	
	</div>							
"""),max_width=800)
end
