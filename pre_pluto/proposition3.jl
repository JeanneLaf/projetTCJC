...
function parse_itinerary(itinerary::JSON3.Object; MAP_BOUNDS=[(46.75,-71.6),(46.96,-71.4)])
	flm = pyimport("folium")
...
!!! info "Proposition 3"
	...
	- **Un** départ du parcours 14X *(Route Ste-Geneviève)* le matin à **10h40**
	- **Un** retour du parcours 14X *(Route Ste-Geneviève)* début PM à **12h45**
	- **Deux** retour du parcours 14 le soir à **16h40** et **18h40**
	```
	Temps ajouté: à calculer;
	Distance ajoutée: à calculer
	```
...
begin
	retourPM= [Dates.format(Time(t),"HH:MM") for t in retour14PM1[:stop_times]]
	splice!(retourPM, 12:1,["--" for i in 1:3])

	AM3 = [Dates.format(Time(t),"HH:MM") for t in depart14XAM2[:stop_times]]
	append!(AM3,["--" for i in 1:4])
	
	prop2_df = DataFrame(
		no = [s[:rt_stop_id] for s in trips3[1][:stops]],
		arrêt = [s[:stop_name] for s in trips3[1][:stops]],
		AM1 = [Dates.format(Time(t),"HH:MM") for t in depart14AM1[:stop_times]],
...
Destinations = Dict(
	...
	heures = ["Aller (~6h30)","Aller (~7h)","Aller (~8h30)", "Aller (~11h)",
			  "Retour (~12h)","Retour (~15h30)","Retour (~16h)","Retour (~18h)"]
WideCell(@htl(
	"""
...
begin
	...
	elseif direction == "Aller (~8h30)"
		heure="08:15"
		response = callAPI(Departs[depart],Destinations[arrivee],heure)
	elseif direction == "Aller (~11h)"
		heure="10:35"
		response = callAPI(Departs[depart],Destinations[arrivee],heure)
	elseif direction == "Retour (~12h)"
		heure = "12:00"
