...
#CARTE DE LA RÉGION
###################
mx = get_map_data("../maps/MRCJCartier.pbf", use_cache=false,only_intersections=false);
...
# ╔═╡ 2f4880d9-b29c-4e16-b81c-0cbdcd36d12a
########
# SBDL
########
begin
	open("../json/route_sbdl.json", "r") do io
		global sbdl = JSON3.read(io)
	end

	route_45_stops = [s for s in sbdl[:itineraries][1][:stops]]
	route_45_stops[end] = RTC["3371"]
	route_45_stops = vcat(route_45_stops, [RTC["1025"], RTC["8002"],RTC["1201"],RTC["2539"]])
	...
	route_45_info = Dict(
	    ...
	    :route_short_name => "45",
	    :route_long_name => "SBDL WEEKEND",
	    :route_type => 3, # 3 = Bus
	    :route_color => sbdl[:route][:route_color],
	    :shape_id => "ROUTE_45",
	    :stops => route_45_stops
	)

	# Calcul du temps de trajet et distances
	route_45 = merge(route_45_info, compute_route(mx,route_45_stops))

	"SBDL temps: $(round(route_45[:total_time]/60,digits=2)) minutes, distance: $(round(route_45[:total_distance]/1000,digits=2)) km"
end

# ╔═╡ 63ac4084-0983-4dc1-8331-2383b4ece153
########
# LAC BEAUPORT
########
begin
	open("../json/route_lb.json", "r") do io
		global lb = JSON3.read(io)
	end

	route_53_stops = [s for s in lb[:itineraries][1][:stops]]
	route_53_stops = vcat(TCJC["40-02"],route_53_stops[15:end])
	...
	route_53_info = Dict(
	    ...
	    :route_long_name => "SBDL WEEKEND",
	    :route_type => 3, # 3 = Bus
	    :route_color => lb[:route][:route_color],
	    :shape_id => "ROUTE_53",
	    :stops => route_53_stops
	)

	# Calcul du temps de trajet et distances
	route_53 = merge(route_53_info, compute_route(mx,route_53_stops))

	"LAC BEAUPORT $(round(route_53[:total_time]/60,digits=2)) minutes, distance: $(round(route_53[:total_distance]/1000,digits=2)) km"
end

# ╔═╡ c5394458-6eec-49ae-94d6-225288e036b9
###############
# STONEHAM
###############
begin
	open("../json/route_stoneham.json", "r") do io
		global stoneham = JSON3.read(io)
	end
	
	route_23_stops = [s for s in stoneham[:itineraries][1][:stops]]
	route_23_stops = vcat([RTC["2539"]],route_23_stops)
	...
	route_23_info = Dict(
	    ...
	    :agency_id => "tests_TCJC",
	    :route_short_name => "23",
	    :route_long_name => "STONEHAM WEEKEND",
	    :route_type => 3, # 3 = Bus
	    :route_color => stoneham[:route][:route_color],
	    :shape_id => "ROUTE_53",
	    :stops => route_23_stops
	)
	# Calcul du temps de trajet et distances
	route_23 = merge(route_23_info, compute_route(mx,route_23_stops))

	"STONEHAM $(round(route_23[:total_time]/60,digits=2)) minutes, distance: $(round(route_23[:total_distance]/1000,digits=2)) km"
end

# ╔═╡ 266f390e-656c-4942-b0fb-010ec2f041dd
begin
	# Nouvel arrêt
	TCJC["VVV"] = Dict(:stop_name => "Village vacances Valcartier", :stop_lat => 46.9400668, :stop_lon => -71.4731970, :rt_stop_id => "VVV")
	
	route_101V_stops = [TCJC["10-07"],TCJC["10-06"],TCJC["10-05"],TCJC["10-04"],TCJC["10-08"],                 TCJC["10-03"],TCJC["10-02"],TCJC["10-01"],TCJC["30-04"],TCJC["30-03"],
	                   TCJC["30-02"],RTC["7000"],RTC["7331"],TCJC["VVV"]]
	
	route_101_stops = [TCJC["10-07"],TCJC["10-06"],TCJC["10-05"],TCJC["10-04"],TCJC["10-08"],                 TCJC["10-03"],TCJC["10-02"],TCJC["10-01"],TCJC["30-04"],TCJC["30-03"],
	                   TCJC["30-02"],RTC["5605"],RTC["2539"]]
	
	
	
	# Informations pour GTFS
	route_101V_info = Dict(
	    :route_id => "TCJC:101V",
	    :agency_id => "tests_TCJC",
	    :route_short_name => "101V",
	    :route_long_name => "TCJC WEEKEND",
	    :route_type => 3, # 3 = Bus
	    :route_color => "8C6CE6", # Bleu-violet
	    :shape_id => "ROUTE_101V",
	    :stops => route_101V_stops
	)
	
	# Calcul du temps de trajet et distances
	route_101V = merge(route_101V_info, compute_route(mx,route_101V_stops))
	...
	route_101_info = Dict(
	    ...
	    :route_color => "8C6CE6", # Bleu-violet
	    :shape_id => "ROUTE_101",
	    :stops => route_101_stops
	)
	
	# Calcul du temps de trajet et distances
	route_101 = merge(route_101_info, compute_route(mx,route_101_stops))

	"FOSSAMBAULT/SCJC/SHANNON $(round(route_101[:total_time]/60,digits=2)) minutes, distance: $(round(route_101[:total_distance]/1000,digits=2)) km"
end
...
begin
	...
	locs = [LLA(mx.nodes[n],mx.bounds) for n in route_101[:all_route]]

	# Stop markers
	for stop in route_101[:stops]
	...
	### SBDL 45 ###
	# Route nodes lattitude and longitude
	locs = [LLA(mx.nodes[n],mx.bounds) for n in route_45[:all_route]]


	# Stop markers
	for stop in route_45[:stops]
		flm.CircleMarker(location=[stop[:stop_lat],stop[:stop_lon]],
				   		 popup=stop[:rt_stop_id]*stop[:stop_name],
						 fill = true,
				   		 radius = 2,
						 color= "#"*route_45[:route_color]
				   ).add_to(m) 
	end
	...
	### LAC BEAUPORT 53 ###
	# Route nodes lattitude and longitude
	locs = [LLA(mx.nodes[n],mx.bounds) for n in route_53[:all_route]]


	# Stop markers
	for stop in route_53[:stops]
		flm.CircleMarker(location=[stop[:stop_lat],stop[:stop_lon]],
				   		 popup=stop[:rt_stop_id]*stop[:stop_name],
						 fill = true,
				   		 radius = 2,
						 color= "#"*route_53[:route_color]
				   ).add_to(m) 
	end
	...
	### STONEHAM 23 ###
	# Route nodes lattitude and longitude
	locs = [LLA(mx.nodes[n],mx.bounds) for n in route_23[:all_route]]

	# Stop markers
	for stop in route_23[:stops]
		flm.CircleMarker(location=[stop[:stop_lat],stop[:stop_lon]],
				   		 popup=stop[:rt_stop_id]*stop[:stop_name],
						 fill = true,
				   		 radius = 2,
						 color = "#"*route_23[:route_color]
				   ).add_to(m) 
	end
