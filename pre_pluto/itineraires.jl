...
# ╔═╡ 1a8ddb69-cd34-48b3-8e80-f4278e244abd
begin
...
Destinations = Dict(
	...
	"UQTR (Campus de Québec)" => (
		latlon = (46.821502, -71.251079),
		type = "Enseignement supérieur"
	),
	...
	"Parc industriel Armand-Viau" => (
		latlon = (RTC["5408"][:stop_lat], RTC["5408"][:stop_lon]),
		type = "Milieux professionnels"
	),
	"Parc industriel Colbert" => (
		latlon = (RTC["2288"][:stop_lat], RTC["2288"][:stop_lon]),
		type = "Milieux professionnels"
	),
	"Espace d'innovation Chauveau" => (
		latlon = (RTC["5995"][:stop_lat], RTC["5995"][:stop_lon]),
		type = "Milieux professionnels"
	),
	...
	heures = ["Aller (~6h)","Aller (~6h30)","Aller (~7h)","Aller (~8h)", "Aller (~11h)",
			  "Retour (~12h)","Retour (~15h30)","Retour (~16h)","Retour (~18h)","Retour (~19h)","Retour (~20h)"]
WideCell(@htl(
	"""
...
# ╔═╡ dfe9ac0d-aea4-4d0f-b478-062e33b5f41e
WideCell(md"""#### Itinéraires 
		 ---""",max_width=800)

# ╔═╡ 7deafec3-be08-45ae-92c6-f4adf33f0f58
begin
